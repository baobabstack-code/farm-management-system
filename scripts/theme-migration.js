...
const MIGRATION_PATTERNS = {
    // Page containers
    'min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-slate-900 dark:to-gray-800': 'page-container',
    ...
    'text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight': 'farm-heading-display',
    'text-lg sm:text-xl font-semibold': 'farm-heading-section',

    // Grids
    'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6': 'farm-grid farm-grid-responsive',
    'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6': 'farm-grid farm-grid-metrics',
...
function analyzeFile(filePath) {
        ...
        if (matches) {
            matches.forEach(match => {
                issues.push({
                    ...
                    pattern: match,
                    suggestion: 'Consider using semantic colors (success, warning, destructive, info, primary, secondary)'
                });
            });
        }
    });
    ...
    if (content.includes('farm-') && !content.includes('farm-theme')) {
        issues.push({
            type: 'missing_import',
            suggestion: 'Consider importing theme components from "@/components/ui/farm-theme"'
        });
    }
...
function generateReport() {
            ...
            issues.forEach((issue, index) => {
                switch (issue.type) {
                    ...
                    case 'color_inconsistency':
                        console.log(`   ${index + 1}. Color inconsistency: ${issue.pattern}`);
    ...
    const pageFiles = fileReports.filter(({ file }) =>
        file.includes('/app/') && file.endsWith('/page.tsx')
    );

    const componentFiles = fileReports.filter(({ file }) =>
        file.includes('/components/') && !file.includes('/ui/')
    );
